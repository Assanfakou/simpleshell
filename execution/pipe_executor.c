...
/*   Updated: 2025/08/09 23:17:45 by rmaanane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_signals_and_pipes(t_cmd *cmd, int *nb_cmds, int *nb_pipes,
		int **pipes)
{
...
void	restore_signal_to_def(void)
{
	signal(SIGINT, SIG_DFL);
	signal(SIGQUIT, SIG_DFL);
}

pid_t	handle_fork_and_execution(t_cmd *cmd, t_env **env, int *pipes,
		int nb_pipes)
{
	t_cmd	*temp;
	pid_t	pid;
